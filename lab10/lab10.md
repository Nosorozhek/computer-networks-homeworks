...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Identification, Header Checksum и периодически Time to Live
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Version, Header Length, Source AAddress, Destination Address, Protocol, Differentiated Services
      - IP адреса отправителя и получателя, версия протокола 
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - увеличивается каждый раз на 1

![](assets/wireshark_first_request.png)
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification разный, потому что он назначается маршрутизатором также каждый раз увеличиваясь на 1
   - TTL одинаковый, потому что роутер отправляет пакеты с дефолтным TTL
...
#### Демонстрация работы
| Бразилия 200.9.221.204    | Мадагаскар 154.126.81.163     |
|---------------------------|-------------------------------|
|![](assets/ping_brazil.png)|![](assets/ping_madagascar.png)|
...
Если раскомментировать строку `socket.set_ttl(1)?;`, то можно будет добиться ошибки Time to Live exceeded in transit, потому что до произвольного хоста нельзя достучаться напрямую, поэтому `ttl=1` недостаточно
![](assets/ping_ttl.png)
