...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   - Мой хост 192.168.5.5
   - IP-адрес назначения 18.244.87.13 ([cloverworks.co.jp](https://cloverworks.co.jp))
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Потому что он работает на сетевом уровне, а порты это абстракция транспортного.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - Тип 8, код 0
   - Контрольная сумма, порядковый номер пакета, идентификатор сессии пинга, данные содержащие timestamp запроса
...
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Тип 0, код 0
   - Контрольная сумма, порядковый номер пакета, идентификатор сессии пинга, данные полученные в запросе
   - По 2 байта на каждое поле

| ![](assets/ping_request.png) | ![](assets/ping_response.png) |
|------------------------------|-------------------------------|
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Размер данных меньше, остальные поля такие же, но, очевидно, с другими значениями
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Содержится оригинальный запрос и поле Unused вместо Identifier и Sequence Number
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Они имеют тип 0 и код 0, а пакеты с ошибками имеют тип 11, src адрес этих пакетов это как раз хост, который мы трэйсрутим 
   - Эти эхо запросы дошли до хоста назначения, поэтому они вернули нормальный ответ, в то время как остальные пакеты не доходили и нам возвращалась ошибка от хоста, где он остановился  
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Канал между host-0.241.48.185.ucom.am и 1.178.76.87
   - Первый расположен в Армении, а второй, судя по информации в интернете, это сервер Amazon, находящийся в Сиэтле

| ![](assets/traceroute_request.png) | ![](assets/traceroute_response.png) |
|------------------------------------|-------------------------------------|
| ![](assets/traceroute_reply.png)   | ![](assets/traceroute_output.png)   |
