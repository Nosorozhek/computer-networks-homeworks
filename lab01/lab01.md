...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Английский (США)
   - Версию браузера, его движка и тип операционной системы
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - IP-адрес моего компьютера 192.168.10.11
...
6. Сколько байтов контента возвращается вашему браузеру?
   - Если считать за контент только полезную нагрузку, то 128 байт (поле Content-Length), 
     если же включать в ответ все поля, то длина всего фрейма 555 байт

| ![GET request](assets/task1_request.png) | ![Server response](assets/task1_response.png) |
|------------------------------------------|-----------------------------------------------|
...
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Угу, там указана дата и время модификации контента на сервере из первого ответа
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - 304 Not Modified
   - Сервер содержимое файла не вернул, тк. он не менялся, браузер достал его из кеша

| ![The first request](assets/task2_request1.png)  | ![The first response](assets/task2_response1.png)  |
|--------------------------------------------------|----------------------------------------------------|
| ![The second request](assets/task2_request2.png) | ![The second response](assets/task2_response2.png) |
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - 1 сообщение, если не считать запрос на получение favicon.ico сайта
   - Номер 30
...
3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - 2 сегмента (`[2 Reassembled TCP Segments (4864 bytes): #36(2896), #39(1968)]`)
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Неа, в заголовке HTTP не содержится информация о более низкоуровневом транспортном протоколе TCP

![Server response](assets/task3_response.png)
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - Было отправлено 4 запроса:
     - запрос самой страницы &mdash; на IP-адрес 128.119.245.12
     - 2 запроса картинок из документа &mdash; на IP-адреса 128.119.245.12 и 2.56.99.24
     - запрос иконки favicon.ico на IP-адрес 128.119.245.12
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Браузер сделал это параллельно, потому что между отправкой запроса на получение картинки pearson.png и ее получением
      (вернее он получил ответ, что картинка теперь лежит по тому же адресу, но ходить надо по протоколу HTTPS), 
      он успел отправить запрос на получение второй картинки, а получил ее уже в конце. 

![Image request](assets/task4_request.png)
...
![Authorization request](assets/task5_request2.png)
